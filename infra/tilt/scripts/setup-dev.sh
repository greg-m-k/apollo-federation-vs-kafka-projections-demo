...
#   ./infra/tilt/scripts/setup-dev.sh --skip-build # Skip Maven pre-build
#   ./infra/tilt/scripts/setup-dev.sh --check-only # Just check prerequisites
...
if [[ "$SKIP_BUILD" == "false" ]]; then
    ...
    for service in "${SERVICES[@]}"; do
        if [[ -d "$service" ]]; then
            ...
            pushd "$MVN_DIR" > /dev/null
            if ./mvnw -f "../../$service/pom.xml" package -DskipTests -q 2>/dev/null; then
                echo " OK"
            else
                echo " FAILED"
                FAILED+=("$service")
...
echo "  Kafka Query: http://localhost:8090"
echo ""
