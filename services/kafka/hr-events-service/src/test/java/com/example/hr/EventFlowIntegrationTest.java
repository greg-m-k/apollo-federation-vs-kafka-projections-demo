...
class EventFlowIntegrationTest {
    ...
    void testCreatePersonFlow_GeneratesOutboxEvent() {
        ...
        .then()
            .statusCode(201)
            .body("name", is("Event Test Person"))
            .extract().response();
        ...
        assertThat(event.payload).contains("Event Test Person");
        assertThat(event.payload).contains("event.test@example.com");
    }
    ...
    void testOutboxEventPayloadContainsAllRequiredFields() {
        ...
        // Verify payload contains all required fields for event consumers
